...
def server(input, output, session):
    ...
    def ctr_and_spend_by_campaign_pie_charts():
        ...
        if not df.empty:
            ...
            # CTR Pie Chart
            axes[0].pie(campaign_ctr, labels=campaign_ctr.index, autopct='%1.1f%%', startangle=90, colors=sns.color_palette('Set2'))
            axes[0].set_title('CTR Distribution by Campaign', fontsize=10)

            # Spend Pie Chart
            axes[1].pie(campaign_spend, labels=campaign_spend.index, autopct='%1.1f%%', startangle=90, colors=sns.color_palette('Set3'))
    ...
    def age_distribution_by_ad_name():
        ...
        axes[i].set_title(f'{metric.capitalize()} Distribution by Age Group', fontsize=12)
...
app_ui = ui.page_fluid(
    ui.panel_title("Government of Kuwait - Social Media Dashboard", "Social Media Dashboard"),
    ui.tags.br(),
    ...
    ui.tags.br(),
    ui.card(
        ui.card_header("Overall Campaign Metrics"),
        ui.card_body(
            ui.output_text("average_metrics")
        ),
